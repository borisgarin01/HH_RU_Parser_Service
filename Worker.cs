...
namespace HH_RU_ParserService
{
        ...
        ILogger<WindowsBackgroundService> logger) : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                {
                    await worker.ImportVacanciesFromHH_RU_ViaPI_ToPostgresAsync();
    ...
    public class Worker
    {
        ...
        public async Task ImportVacanciesFromHH_RU_ViaPI_ToPostgresAsync()
        {
            Root root = await httpClient.GetFromJsonAsync<Root>("https://api.hh.ru/vacancies?text=C%23&search_field=name&employment=full&schedule=remote");
